...
bool CSynthesizer::Generate(double * frame)
{
	...
	while (m_currentNote < (int)m_notes.size())
	{
		...
		{
			m_waveinstfactory.SetBPM(GetBeatsPerMinute());
			m_waveinstfactory.SetNote(note);
			instrument = m_waveinstfactory.CreateInstrument(note->Name());
		}
		else if (note->Instrument() == L"Drum")
		{
			m_drumsynthfactory.SetBPM(GetBeatsPerMinute());
			m_drumsynthfactory.SetNote(note);
			instrument = m_drumsynthfactory.CreateInstrument();
		}
		else if (note->Instrument() == L"Noise Gate")
		{
			m_noiseGate.SetNote(note);
		}
		else if (note->Instrument() == L"Compressor")
		{
			m_compressor.SetNote(note);
		}
   ...
   // Phase 3b: Effects
   //
	double effectOutput[NUMEFFECTSCHANNELS][2];
	for (int i = 0; i < NUMEFFECTSCHANNELS; i++)
	{
		for (int c = 0; c < GetNumChannels(); c++)
		{
	...
	{
		effectOutput[0][i] += channelframes[0][i];
	}

	// Compressor and noise gate in serialization
	double intermediate_send1[2];
	intermediate_send1[0] = 0.0;
	intermediate_send1[1] = 0.0;

	m_compressor.Process(channelframes[1], intermediate_send1/*effectOutput[1]*/);
	m_noiseGate.Process( intermediate_send1 /*channelframes[1]*/, effectOutput[1]);


	// reverb
	m_reverb.Process(channelframes[2], effectOutput[2]);

	//
	// sum the channel frames and copy them into the output
	//
	for (int i = 0; i < NUMEFFECTSCHANNELS; i++)
	{
		for (int c = 0; c < GetNumChannels(); c++)
		{
			frame[c] += effectOutput[i][c];
		}
	}
...
void CSynthesizer::XmlLoadInstrument(IXMLDOMNode * xml)
{
	...
	for (; node != NULL; NextNode(node))
	{
		...
		if (name == L"note")
		{
			XmlLoadNote(node, instrument, inst_name);
		}
	}

}
void CSynthesizer::XmlLoadNote(IXMLDOMNode * xml, std::wstring & instrument , std::wstring& name)
{
	m_notes.push_back(CNote());
	m_notes.back().XmlLoad(xml, instrument, name);
}